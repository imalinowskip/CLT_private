...
variable {Ω' : Type*} [MeasurableSpace Ω'] --[TopologicalSpace Ω]
variable {Q : ProbabilityMeasure Ω'}
variable {d : ℕ+}
...
lemma aemeasurable_dotProduct {n : ℕ} {μ : Measure Ω} (hX : Measurable (Xn n)) (t : EuclideanSpace ℝ (Fin d)):
...
lemma charFun_tendsto_if_inner_tendsto (hX : Measurable X) (hXn : ∀ n, Measurable (Xn n)):
  ...
  by
    ...
    have h_eq3 : charFun (Q.map hX.aemeasurable) t = ∫ x, Complex.exp (⟪X x, t⟫ * Complex.I) ∂Q :=
      by
        simp only [charFun]
        apply MeasureTheory.integral_map
        · exact hX.aemeasurable
        · exact φ.continuous.stronglyMeasurable.aestronglyMeasurable
    have h_eq_final : charFun (Q.map hX.aemeasurable) t = ∫ x, Complex.exp (x * Complex.I) ∂(Q.map (aemeasurable_dotProduct' hX t)).toMeasure :=
      by
        rw [h_eq3]
        rw [eq_comm]
        apply MeasureTheory.integral_map
    ...
    let h_lim (f : ℝ →ᵇ ℝ) : 0 ≤ f → atTop.limsup (fun n => ∫ x, f x ∂ (↑(P.map (aemeasurable_dotProduct (hXn n) t)))) ≤ ∫ x, f x ∂ (↑(Q.map (aemeasurable_dotProduct' hX t))) :=
      by
        ...
        have hf : Tendsto (fun n => ∫ x, f x ∂ (↑(P.map (aemeasurable_dotProduct (hXn n) t)))) atTop
                        (𝓝 (∫ x, f x ∂ (↑(Q.map (aemeasurable_dotProduct' hX t))))) :=
          by
      ...
      Tendsto (fun n => ∫ (x : ℝ), ψ x ∂ (↑(P.map (aemeasurable_dotProduct (hXn n) t)))) atTop (𝓝 (∫ (x : ℝ), ψ x ∂ (↑(Q.map (aemeasurable_dotProduct' hX t))))) :=
        by
          ...
          have h1 : Tendsto (fun n => Complex.ofReal (∫ (x : ℝ), ψ_re_bcf x ∂ (↑(P.map (aemeasurable_dotProduct (hXn n) t))))) atTop
            (𝓝 (↑(∫ (x : ℝ), ψ_re_bcf x ∂ (↑(Q.map (aemeasurable_dotProduct' hX t)))))) :=
          ...
          have h2 : Tendsto (fun n => Complex.ofReal (∫ (x : ℝ), ψ_im_bcf x ∂ (↑(P.map (aemeasurable_dotProduct (hXn n) t))))) atTop
            (𝓝 (↑(∫ (x : ℝ), ψ_im_bcf x ∂ (↑(Q.map (aemeasurable_dotProduct' hX t)))))) :=
    ...
    have h_char_eq_lim : charFun (Q.map hX.aemeasurable) t = ∫ x, ψ x ∂(↑(Q.map (aemeasurable_dotProduct' hX t))) :=
      h_eq_final
    ...
    have h_lim_eq : ∫ x, Complex.exp (x * Complex.I) ∂((Q.map (aemeasurable_dotProduct' hX t)).toMeasure) =
                ∫ x, ψ x ∂(↑(Q.map (aemeasurable_dotProduct' hX t))) :=
      by rw [← hψ_eq_cexp]
